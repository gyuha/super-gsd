...
## Current State (v1.1 shipped 2026-05-20)

- **Commands**: 11개 sg- 명령 (v1.0의 9개 + `sg-health`, `sg-quick`, `sg-update` 추가; 그 외 sg-status/sg-start/sg-review/sg-learn/sg-ship/sg-complete/sg-execute/sg-plan/sg-explore/sg-lessons)
- **Hooks**: `hooks/hooks.json` (Stop + SubagentStop), `stop_hook.py`, `transcript_matcher.py` (IMPLEMENTATION_SIGNALS 정밀화 + HOOKIFY_SIGNALS의 `hookify` 패턴 제거)
- **State**: 7-state storage(`init|gsd-plan|superpowers|review|hookify|ship|complete`) ↔ 4-state display 매핑. STATE.md `Phase:` single source of truth.
- **Session restore**: sg-start가 STATE.md/HANDOFF.md를 자동 감지해 Resume/Start new milestone/Cancel 3-옵션 분기 (HANDOFF.md read-only 보존)
- **Self-diagnosis**: `sg-health`로 설치/훅/스키마 한 번에 진단
- **Lessons loop**: `.planning/lessons/{NN}-{YYYY-MM-DD}.md` 자동 생성, `sg-plan` Step 0 자동 재표시 (v1.0 유지)
- **Hookify rules**: `.claude/hookify.*.local.md` 15개 (gitignored, 로컬 자동 가드 — grep BRE 이스케이프, STATE/HANDOFF drift, sg-complete pending phase 등)

## Current Milestone: v1.2 Self-Contained Retrospection

**Goal:** `sg-learn`이 외부 `hookify:hookify` Skill에 위임하는 현재 구조를 내장 retrospection으로 교체. super-gsd가 hookify 플러그인 없이 단독으로 회고·패턴 추출·실수 재발 방지 가드를 모두 수행한다. 사용자가 같은 실수를 반복하지 않고 지속적으로 개발할 수 있도록, 한 가지 lens가 아닌 여러 관점에서 분석한다.

**Target features:**

1. **`sg-retro` 내장 Skill** — `skills/sg-retro/` 디렉터리에 자체 Skill. Start/Stop/Continue, 4Ls, Sailboat, Five Whys, Decisions/Surprises/Patterns/Mistakes 등 다관점 lens를 한 도구에서 처리
2. **내장 conversation analyzer** — frustration signals · correction patterns · repeated issues · validated successes를 session transcript에서 자체 추출 (hookify 의존 없음)
3. **자체 rule runner** — super-gsd가 PreToolUse hook을 등록해 `.claude/sg-rule.*.local.md` 파일을 직접 실행. hookify 플러그인 미설치 환경에서도 가드 동작
4. **lessons 영속화 확장** — 다관점 retro 결과를 phase·milestone별 묶음으로 `.planning/lessons/`에 저장. `sg-plan` Step 0 자동 주입 흐름 유지
5. **재발 방지 가드** — 과거 catch된 top-N 패턴을 `sg-plan`/`sg-execute` 시작 시 우선순위로 노출 (단순 dump가 아닌 weighted ranking)
6. **`sg-learn` 라우팅 전환 + hookify 의존성 제거** — `hookify:hookify` 호출을 내장 `sg-retro` Skill 호출로 교체. README/prerequisites에서 hookify 항목 삭제

**Deferred from v1.1 (v1.2에서 처리 또는 추가 deferred):**
- Phase 08 Task 2 7-시나리오 manual regression run — sg-retro lens 중 하나로 흡수 검토
- 9개 quick-task SUMMARY retroactive 작성 cleanup — sg-retro 가 자동 SUMMARY 생성 능력 갖추면 흡수
- sg-status 7일 초과 stale 세션 경고 — v1.2 재발 방지 가드 일부로 검토
- 나머지(sg-learn hookify 자동 행 추가, --json 플래그, cwd walk-up, sg-start 비대화형) — v1.3 후보
