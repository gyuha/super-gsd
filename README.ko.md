...
**Phase 5에서 학습 루프가 완성됐다.** Hookify 회고 출력이 `.planning/lessons/`에 자동 저장되고, 다음 GSD plan 단계 시작 시 자동으로 주입된다. 전체 사이클을 커버하는 11개 슬래시 명령이 제공된다. 명령 빠른 참조는 **명령어** 섹션을, 전체 명령 레퍼런스는 `docs/COMMANDS.md`를 참고한다.
...
`sg-status`는 언제든 현재 위치를 확인하는 데 사용할 수 있다.
...
| `/super-gsd:sg-execute` | 현재 단계 계획을 패키징하여 Superpowers에 인계 | `sg-plan` 완료 후 |
...
| `/super-gsd:sg-ship` | `gsd-ship`을 통해 현재 마일스톤 완료 및 배포 | 학습 캡처 후 |
...
# 7. 배포 — gsd-ship을 통해 마일스톤 완료 및 병합
/super-gsd:sg-ship
```
