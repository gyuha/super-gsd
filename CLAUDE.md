...
- **Rules**: `.claude/sg-rule.*.local.md` — YAML frontmatter(name, enabled, event, pattern/conditions, action)가 있는 Markdown. `rule_runner.py`가 평가
...
`rule_runner.py`는 `prompt` 이벤트는 지원하지 않는다.
...
- `rule_runner.py` — PreToolUse 이벤트에서 `.claude/sg-rule.*.local.md` 규칙을 평가한다.
