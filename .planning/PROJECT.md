...
## Current Milestone: v2.2 sg-next Auto-Advance

**Goal:** sg-next 명령 하나로 현재 워크플로우 단계를 감지하고 다음 sg-* 명령을 자동 실행한다.

**Target features:**
- sg-status 기반 단계 감지 (HANDOFF.md + STATE.md)
- 단계별 라우팅 테이블로 다음 sg-* 명령 자동 invoke
- 1줄 진행 상황 출력 후 확인 없이 즉시 실행
- 모호한 상황(complete, init)에서 선택지 제시

---

## Current State (v2.1 shipped 2026-05-23)
...
- **State**: 7-state storage(`init|gsd-plan|ui-plan|superpowers|review|hookify|ship|complete`) ↔ 4-state display 매핑
- **Skills 품질**: 17개 description GOOD 등급 + sg-retro 390줄 축소 (v2.1 Phase 25 완료)
- **Bug fixes**: sg-execute/sg-retro 소수점 phase 지원, sg-status/sg-start ui-plan stage 지원, sg-learn HANDOFF 기록 타이밍 수정
- **Dependencies**: GSD + Superpowers 필수. Hookify Optional (sg-retro 내장으로 대체)
- **Version**: 0.0.31

## Next Milestone Goals

**v2.3** (planned) — TBD after v2.2 complete
...
---
*Last updated: 2026-05-23 — v2.2 sg-next Auto-Advance 마일스톤 시작. v2.1 완료 (QUAL-06: 17개 description GOOD 등급 교체 + sg-retro 390줄 축소 + 4개 버그 수정)*
