...
22개의 SKILL.md 파일이 각각 하나의 `/super-gsd:sg-*` 슬래시 명령을 정의한다. 대부분은 GSD → Superpowers → sg-retro 워크플로우 명령이고, sg-retro(회고), sg-parallel-execute(병렬 실행), sg-ui-plan(UI 설계), sg-next(자동 진행), sg-quick·sg-phase·sg-cleanup·sg-health·sg-update·sg-setup 등 보조 명령이 포함된다(전체 목록은 README Commands 표 참고). 이 중 11개는 `.agents/skills/`에도 미러로 존재해 Codex/Gemini에서 `$sg-*`로 호출된다. Claude Code가 SKILL.md를 실행 지시문으로 해석하며, GSD skill, Superpowers skill, 또는 Node.js 스크립트(.cjs)를 호출해 단계 간 인계를 수행한다.
