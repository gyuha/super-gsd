...
- **`skills/` + `.agents/` 쌍 커버** — `skills/*/SKILL.md`를 수정·변환하는 플랜을 작성할 때, 동일 이름의 `.agents/skills/*/SKILL.md`가 존재하는지 확인하고 해당 파일도 플랜에 명시적으로 포함한다. 같은 패턴 변환이라도 `.agents/` 파일은 별도 task 항목 또는 `files` 목록에 열거해야 한다. 누락 시 plan-checker가 블로커로 탐지한다 (Phase 32 Medium-1).
<!-- GSD:conventions-end -->
