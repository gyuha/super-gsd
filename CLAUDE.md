...
- **Tech stack**: Claude Code 플러그인 시스템 (skills + hooks). Bash/Python/Markdown 위주.
...
18개의 SKILL.md 파일이 각각 하나의 `/super-gsd:sg-*` 슬래시 명령을 정의한다. 14개는 GSD → Superpowers → sg-retro 워크플로우 명령이고, sg-retro(회고), sg-parallel-execute(병렬 실행), sg-ui-plan(UI 설계), sg-next(자동 진행)이 추가로 포함된다. Claude Code가 SKILL.md를 실행 지시문으로 해석하며, GSD skill, Superpowers skill, 또는 Python 스크립트를 호출해 단계 간 인계를 수행한다.
