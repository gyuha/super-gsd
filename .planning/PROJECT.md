...
**Current milestone:** v2.11 Add TDD workflow (sg-tdd) — started 2026-06-01

## Current Milestone: v2.11 Add TDD workflow (sg-tdd)

**Goal:** sg-execute와 sg-review 사이에 Superpowers `test-driven-development`를 오케스트레이션하는 red-green-refactor 검증 게이트 `sg-tdd`를 추가한다.

**Target features:**
- sg-tdd 명령 (Superpowers TDD 오케스트레이션, Non-invasive) — sg-execute 산출물에 대한 red-green-refactor 검증 루프
- config 플래그 `super_gsd.tdd_mode` (on/off)로 단계 gating
- sg-next auto-advance 라우팅에 sg-tdd 포함 (tdd_mode on일 때)
- stop_hook/transcript_matcher sg-tdd stage 전이 신호 감지
- `.agents/skills/sg-tdd/SKILL.md` 미러 (Codex/Gemini)
- HANDOFF.md sg-tdd stage 기록
- 문서 갱신 (README/README.ko.md Commands 표, CLAUDE.md 아키텍처)

**Key context:** sg-tdd는 구현(sg-execute) 이후 단계이므로 고전적 "테스트 우선" TDD가 아니라 "TDD 준수 검증 게이트"다 — 테스트가 구현 없이는 실패하는지(의미 없는 통과 테스트 차단)를 확인하고 refactor 패스를 돌린다. Non-invasive 제약 유지(Superpowers/GSD 파일 미수정, 외부 오케스트레이션만).
