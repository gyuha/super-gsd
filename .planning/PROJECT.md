...
## Current Milestone: v2.8 Team Collaboration Support

**Goal:** 팀원이 동일 GSD 프로젝트에서 phase 단위로 병렬 작업할 때 파일 충돌 없이 협업할 수 있도록 git-native 워크플로우를 super-gsd에 통합한다.

**Target features:**
- HANDOFF.md에 user 컬럼 추가 (git user.name 기반 자동 기록)
- sg-status --team: phase/* 브랜치 분석으로 팀 현황 표시
- sg-execute: phase 시작 시 브랜치 자동 생성 제안
- sg-complete (phase): PR 생성 명령 안내 (gh CLI)
- TEAM.md 컨벤션 문서 + README 팀 섹션
