...
새 마일스톤 시작부터 완료 및 다음 마일스톤 시작까지 전체 GSD → Superpowers → sg-retro 사이클을 커버하는 21개 슬래시 명령이 제공된다. `sg-next`를 언제든 실행하면 현재 단계를 자동 감지하여 다음 명령을 기억할 필요 없이 바로 실행한다. 명령 빠른 참조는 **명령어** 섹션을, 전체 명령 레퍼런스는 `docs/COMMANDS.md`를 참고한다.

## 워크플로우

```
[ 수동 진입 ]                          [ sg-plan 이후부터 sg-next 자동 체인 ]
...
`sg-next`는 HANDOFF.md/STATE.md를 읽어 현재 단계를 감지하고 체인의 다음 명령을 자동 실행한다 (`gsd-plan → sg-execute`, `parallel/execute → sg-review`, `review → sg-learn`, `sg-retro → sg-ship`, `ship → sg-plan {다음 phase}` 또는 `sg-complete`, `complete → sg-new` via AskUserQuestion). 진입 명령(`sg-start`, `sg-explore`, 첫 `sg-plan`)은 자동 체인 시작 전 **수동**으로 호출한다.
