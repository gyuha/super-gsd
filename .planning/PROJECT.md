...
v1.2에서 확인된 deferred 항목과 잠재적 개선 방향:

- MIGRATION-02 e2e manual checklist 검증 (hookify 미설치 환경에서 sg-learn 테스트)
- sg-status "hookify" stage 표시 → "sg-retro" 또는 "retro"로 rename (하위 호환성 검토 필요)
- sg-health hookify Optional 플래그 반영 (`[OPTIONAL]` 표시)
