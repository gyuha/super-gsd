---
name: sg-toggle-review
description: Toggle the sg-review stage on or off in the super-gsd workflow. Flips super_gsd.skip_review in .planning/config.json so sg-next / sg-status / the stop hook include or skip code review after implementation. Accepts on|off, or no argument to flip.
argument-hint: "[on|off] - 'on' includes review, 'off' skips it, empty flips the current state."
---
...
<objective>
Enable or disable the sg-review stage in the super-gsd workflow by writing the super_gsd.skip_review boolean in .planning/config.json. ON means review runs (skip_review=false, the default); OFF means review is skipped (skip_review=true). With no argument, flip the current value. All other config keys and 2-space formatting are preserved.
</objective>
...
2. **Read-modify-write config.json (macOS compatible — node only, no jq).**
   The underlying flag is `skip_review` (inverted: review ON ⇒ skip_review=false). The effective post-implementation sequence is printed for confirmation.
   ```bash
   ...
   const cur=!!cfg.super_gsd.skip_review;
   const d=process.env.DESIRED;
   let skip; if(d==="on")skip=false; else if(d==="off")skip=true; else skip=!cur;
   cfg.super_gsd.skip_review=skip;
   fs.mkdirSync(path.dirname(p),{recursive:true});
   fs.writeFileSync(p, JSON.stringify(cfg,null,2)+"\n");
   const tdd=!!cfg.super_gsd.tdd_mode, sr=skip, sl=!!cfg.super_gsd.skip_learn;
...
3. **Report the new state in the user's language.**
   Surface the result as prose in the user's input language. Keep the command token `sg-review`, the flag name `skip_review`, and the stage sequence tokens (`execute → … → ship`) verbatim in English.
   - When `STATE=on`: tell the user the `sg-review` stage is now **included** in the workflow.
   - When `STATE=off`: tell the user the `sg-review` stage is now **skipped**.
   - Always show the resulting workflow sequence from `path:` so the user can confirm.
</process>

<success_criteria>
1. With no argument, `super_gsd.skip_review` is flipped relative to its current value (absent treated as false).
2. With `on`/`off`, the stage is set to included/skipped respectively (skip_review=false/true), regardless of prior value (idempotent).
