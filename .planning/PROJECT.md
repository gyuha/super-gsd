...
**Latest shipped milestone:** v2.9 Retro UX Simplification (2026-05-31)
**Current milestone:** v2.10 Plan-Phase Ambiguity Grilling — defining requirements

## Current Milestone: v2.10 Plan-Phase Ambiguity Grilling

**Goal:** `sg-plan`이 discuss subagent를 호출하기 전, 메인 컨텍스트에서 grill-me 원칙으로 사용자를 모호함이 해소될 때까지 질문하여 계획 입력의 불확실성을 제거한다.

**Target features:**
- discuss subagent 호출 전 메인 컨텍스트의 무제한 grill 단계 (한 번에 하나씩 질문)
- 각 질문마다 권장 답변 제시
- 코드베이스로 답 가능한 질문은 묻지 않고 직접 탐색
- 설계 트리 분기를 따라 의존성 순차 해소
- "모호함 없음" → Claude 합의 요약 → 사용자가 확정/추가질문으로 최종 결정
- grill 합의 결과를 gsd-discuss-phase / CONTEXT 입력으로 전달

**Key context:** Non-invasive 제약 유지(gsd-discuss-phase 미수정). 참고: mattpocock grill-me skill. `.agents/skills/sg-plan/SKILL.md` 쌍 파일 동반 변경 필요.
