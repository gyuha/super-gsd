...
3. **package.json 업데이트** — `version` 필드를 새 버전으로 교체한다.
4. **CHANGELOG.md 업데이트** — 파일 상단(첫 번째 `## [x.x.x]` 블록 바로 위)에 새 버전 섹션을 삽입한다. 형식:
   ```
   ...
   ```
5. **git commit** — 변경된 세 파일을 스테이징하고 커밋한다. 메시지: `chore(release): bump version to NEW_VERSION`
6. **git push** — 현재 브랜치를 원격에 push한다.
