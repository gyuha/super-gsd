...
- **Dependencies**: `claude-plugins-official/superpowers`, `get-shit-done-cc` (또는 동등 GSD 설치).
...
- **Plugin manifest**: `.claude-plugin/plugin.json` — 14개 명령 경로, skills 디렉토리, marketplace 메타데이터 정의
...
- **Hooks**: `hooks/*.py` — Python 3. Claude Code의 PreToolUse/Stop/SubagentStop 이벤트에서 실행되며 JSON으로 응답
- **Rules**: `.claude/hookify.*.local.md`, `.claude/sg-rule.*.local.md` — YAML frontmatter(name, enabled, event, pattern/conditions, action)가 있는 Markdown. `rule_runner.py`가 평가
- **Lessons**: `.planning/lessons/*.md` — phase 회고 결과. `lessons_ranker.py`가 frequency·recency·severity 기반 weighted score로 순위를 매겨 `sg-plan`에 주입
<!-- GSD:stack-end -->
...
`rule_runner.py`는 hookify 미설치 환경에서만 동작하며, `prompt` 이벤트는 지원하지 않는다.

### macOS 셸 이식성

hooks와 commands의 Bash 스니펫은 macOS(BSD 도구)와 Linux(GNU 도구) 모두에서 실행된다.
...
플러그인은 세 개의 레이어로 구성된다.

**1. Commands 레이어** (`commands/*.md`)

각 파일이 하나의 `/super-gsd:sg-*` 슬래시 명령을 정의한다. Claude Code가 이 Markdown을 실행 지시문으로 해석한다. 명령은 GSD skill, Superpowers skill, 또는 Python 스크립트를 호출해 단계 간 인계를 수행한다.

**2. Hooks 레이어** (`hooks/`)

- `stop_hook.py` — Stop/SubagentStop 이벤트에서 `transcript_matcher.py`로 신호를 감지해 `systemMessage`로 다음 단계를 안내한다. `.planning/config.json`의 `super_gsd.auto_advance: false`로 비활성화 가능.
- `rule_runner.py` — PreToolUse 이벤트에서 `.claude/hookify.*.local.md`와 `.claude/sg-rule.*.local.md` 규칙을 평가한다. hookify 플러그인이 설치된 환경에서는 자동으로 skip한다.
- `lessons_ranker.py` — CLI 도구. `--top N`으로 weighted top-N 패턴을 JSON lines로 출력하거나 `--archive`로 마일스톤 아카이브를 생성한다.

**3. Skills 레이어** (`skills/`)

`skills/sg-retro/SKILL.md` — 6가지 회고 렌즈(SSC, 4Ls, DSPM, Sailboat, Five Whys, Conversation Analyzer)를 지원하는 구조화 회고 스킬. 결과를 `.planning/lessons/{NN}-{YYYY-MM-DD}.md`에 저장한다.
...
```
| timestamp | phase-slug | from_stage | to_stage | commit_sha |
```

`stop_hook.py`는 이 파일의 마지막 행을 읽어 현재 단계와 이전 상태를 파악한다. **행을 수정하지 말고 항상 끝에 추가만 한다.**

**환경 변수**

- `CLAUDE_PLUGIN_ROOT` — hooks에서 플러그인 루트 경로를 찾는 데 사용. 부재 시 `__file__` 기반 경로로 폴백.

**규칙 파일 우선순위**

`sg-rule.*.local.md` (priority 2) > `hookify.*.local.md` (priority 1). 이름이 충돌할 경우 우선순위가 높은 파일이 채택된다.
