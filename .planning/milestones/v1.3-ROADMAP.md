...
- [x] **Phase 14: Codex 진입점 + .agents/skills/** — AGENTS.md 재작성(Codex 어휘) + .agents/skills/ 스킬 6개 신규 생성
- [x] **Phase 15: 플랫폼별 훅 설정 + Python 픽스** — .codex/hooks.json + .gemini/settings.json 신규 생성 + hooks/*.py 경로 폴백 수정
- [x] **Phase 16: README Multi-Platform 섹션** — 플랫폼별 설치 가이드 + 기능 델타 테이블 추가
