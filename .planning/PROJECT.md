...
## Current Milestone: v2.4 Hooks Node Migration

**Goal:** Python 의존성을 완전히 제거하고 Node.js만으로 실행되는 플러그인으로 전환한다.

**Target features:**
- `hooks/*.py` 4개 파일(`stop_hook`, `rule_runner`, `lessons_ranker`, `transcript_matcher`)을 순수 JS + CommonJS(`.cjs`)로 재작성 — 외부 의존성 0
- `hooks/hooks.json`, `.codex/hooks.json`, `.gemini/settings.json` 명령을 `python3 → node`로 교체
- Skill 내 인라인 `python3 -c` 호출 및 `python3 hooks/lessons_ranker.py` 호출을 `node -e` / `jq` / `node hooks/*.cjs`로 교체 (`.agents/skills/` 사본 포함)
- 문서 갱신 (CLAUDE.md, README.md, README.ko.md, CHANGELOG.md)
- 마일스톤 마지막 phase에서 `.py` 파일 일괄 삭제 (원자적)

**Key context:** 사용자가 Node만 설치된 환경에서도 즉시 실행할 수 있도록 의존성 축소. Claude Code 자체가 Node 18+를 요구하므로 Node는 사실상 보장됨. v2.3 (Phase 27, Ship ready)는 독립적으로 두고 v2.4를 별도 마일스톤으로 진행한다.

**Phase 번호:** Phase 28부터 (이어서 — reset 없음).

- 이후 후보: Multi-Platform Support, Team Agent Parallel Execution, quick tasks 정리
...
---
*Last updated: 2026-05-25 — v2.4 Hooks Node Migration 마일스톤 시작. Python 의존성 제거 목표, hooks/*.py 4개 → *.cjs 재작성 + 설정/스킬/문서 일괄 교체*
