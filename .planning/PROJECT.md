...
**Latest shipped milestone:** v2.10 Plan-Phase Ambiguity Grilling (2026-05-31)
**Current milestone:** v2.11 use-tdd Workflow Mode (started 2026-05-31)

## Current Milestone: v2.11 use-tdd Workflow Mode

**Goal:** `sg-use-tdd` 토글로 TDD 모드를 켜면, `sg-execute`가 실패 테스트를 먼저 작성하도록 Superpowers `test-driven-development` 스킬을 핸드오프 프롬프트에 강제 주입하고, `sg-review`에서 테스트가 통과하지 못하면 사용자 확인 후 `sg-execute`로 되돌려 수정·재작성하는 루프를 만든다. 마커(`.planning/USE-TDD`)가 없으면 기존 동작을 그대로 유지하는 opt-in 기능이다.

**Target features:**
- `sg-use-tdd` 토글 명령 + `.planning/USE-TDD` 마커 파일 (Codex/Gemini용 `.agents/skills/` 미러 포함)
- `sg-execute` TDD 주입 — 마커가 있을 때만 핸드오프 프롬프트에 test-first 지시 추가 (비침투, opt-in)
- `sg-review` 테스트 실패 감지 → 사용자 확인 → `sg-execute` 재실행 루프 (무한 루프 방지를 위한 횟수 제한)
- README.md / README.ko.md 워크플로우 다이어그램·Commands 표에 TDD 단계 추가

**Key context:** 비침투적 orchestrator 원칙 유지 — Superpowers의 `test-driven-development` 스킬을 재사용(재발명 금지). 명령 prefix는 프로젝트 컨벤션상 `sg-` 강제(`sg-use-tdd`). `sg-execute`·`sg-review`는 `.agents/skills/`에도 미러가 존재하므로 pairwise 동기화 필수.

## Next Milestone Goals

v2.11 이후 후보: v2.10 회고에서 이월된 plan-lint 도입(step 번호 산술-순서 일관성 점검), 두 sg-plan SKILL.md 구조 divergence 정합, 미러 파일 분량 비대칭 검토.

## Completed Milestone: v2.10 Plan-Phase Ambiguity Grilling (2026-05-31)

**Goal:** `sg-plan`이 discuss subagent를 호출하기 전, 메인 컨텍스트에서 grill-me 원칙으로 사용자를 모호함이 해소될 때까지 질문하여 계획 입력의 불확실성을 제거한다.

**Delivered:**
- Phase 45: sg-plan Grilling Step (GRILL-01~06) — discuss Agent 호출 전 grill 단계 (`.claude` Step 1.5 AskUserQuestion / `.agents` Step 1.2 프로즈 폴백), 코드베이스 우선 탐색 + 한-번-하나씩 질문 + 권장 답변 동반 + 설계 트리 순차 해소 + 사용자 확정 종료 게이트, 합의를 discuss Agent 프롬프트에 locked context로 inline 주입 (Non-invasive), 두 SKILL.md pairwise sync

**Key context:** Non-invasive 제약 유지(gsd-discuss-phase 미수정). 참고: mattpocock grill-me skill.

## Completed Milestone: v2.9 Retro UX Simplification (2026-05-31)

**Goal:** sg-retro/sg-learn의 lens 선택 마찰을 제거하고 회고 결과를 한눈에 이해할 수 있게 만든다.

**Delivered:**
- Phase 42: Smart Default Lens + Consolidation (LENS-01, LENS-02) — smart default `dspm+ssc`, 6→3 lens(ssc/dspm/analyze), dropped lens reject
- Phase 43: --pick Flag + Display Polish (LENS-03, DISPLAY-01, DISPLAY-02) — token-position-free `--pick` multiSelect, 🔴 P1 emoji prefix, `_Intent: ..._` lens header line
- Phase 44: Documentation Sync (DOC-01) — README/README.ko.md Commands 테이블 갱신 + sg-retro SKILL.md frontmatter --pick 명시 + .planning/TEAM.md retrospective workflow 섹션
- Quick task `3d373d8`: sg-review auto-commit gate when BASE==HEAD + dirty working tree (Phase 42/43 retro P1 #2 closure)
...
### Completed (v2.10 — Phase 45, 2026-05-31)

- ✓ GRILL-01: sg-plan이 discuss 호출 전 메인 컨텍스트에서 한 번에 하나씩 질문하는 grill 단계 수행 (질문 수 무제한) — v2.10 Phase 45
- ✓ GRILL-02: 각 질문마다 Claude가 권장 답변을 함께 제시 — v2.10 Phase 45
- ✓ GRILL-03: 코드베이스 탐색으로 답할 수 있는 질문은 묻지 않고 직접 탐색해 해소 — v2.10 Phase 45
- ✓ GRILL-04: 설계 트리 분기를 따라 의존성을 순차적으로 해소 (이전 답이 다음 질문 좌우) — v2.10 Phase 45
- ✓ GRILL-05: Claude가 모호함 해소 판단 시 합의 요약 제시 → 사용자가 확정/추가질문으로 종료 최종 결정 — v2.10 Phase 45
- ✓ GRILL-06: grill 합의 결과(결정·제약)가 gsd-discuss-phase / CONTEXT 입력으로 전달 — v2.10 Phase 45
...
---
*Last updated: 2026-05-31 after v2.11 milestone start — use-tdd Workflow Mode 목표 설정. Version 0.0.54.*
