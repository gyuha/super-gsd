...
**Current milestone:** (none — run `/super-gsd:sg-new` to start the next milestone)

## Next Milestone Goals

다음 마일스톤은 아직 정의되지 않았다. `/super-gsd:sg-new`로 새 마일스톤을 시작하면 questioning → requirements → roadmap 순으로 정의된다. v2.10 회고에서 이월된 후보: plan-lint 도입(step 번호 산술-순서 일관성 점검), 두 sg-plan SKILL.md 구조 divergence 정합, 미러 파일 분량 비대칭 검토.
