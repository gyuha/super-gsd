...
## Current Milestone: v2.6 Codex/Gemini 설치 UX 개선

**Goal:** Codex/Gemini CLI에서 super-gsd 설치를 GSD(npm) · Superpowers(marketplace) 수준으로 단순화한다 — 현재 4개 cp 명령을 1단계로 줄인다.

**Target features:**
- 단일 설치 명령 — install.sh 스크립트 또는 npx 기반 installer (GSD npm 패턴 참조)
- `$sg-setup` 인세션 스킬 — Codex 세션 내부에서 파일 복사 자동화
- AGENTS.md + README.md 설치 가이드 개선 — Verify install 체크리스트 추가

## Current State (v2.5 shipped 2026-05-26)
