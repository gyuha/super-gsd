...
새 마일스톤 시작부터 완료 및 다음 마일스톤 시작까지 전체 GSD → Superpowers → sg-retro 사이클을 커버하는 25개 슬래시 명령이 제공된다. `sg-next`를 언제든 실행하면 현재 단계를 자동 감지하여 다음 명령을 기억할 필요 없이 바로 실행한다. 명령 빠른 참조는 **명령어** 섹션을, 전체 명령 레퍼런스는 `docs/COMMANDS.md`를 참고한다.

## 워크플로우

워크플로우는 **수동 진입**(`sg-start`/`sg-new`, `sg-explore`, 첫 `sg-plan`을 직접 실행)과 `sg-execute`부터 `sg-next`가 끌고 가는 **자동 체인**으로 나뉜다. `sg-tdd`, `sg-review`, `sg-learn` 단계는 **선택**이다 — `sg-toggle-*`로 끄면(`.planning/config.json`의 `tdd_mode` / `skip_review` / `skip_learn` 설정), `sg-next`·`sg-status`·stop 훅이 모두 비활성 단계를 건너뛰고 다음 비-생략 단계로 직행한다. `sg-start`는 새 프로젝트/마일스톤 시작 시 멀티셀렉트로 세 가지를 한 번에 설정할 수도 있다. 배포된 각 phase는 `sg-plan`으로 되돌아가며(교훈이 다음 plan에 주입됨), 마일스톤을 닫을 때는 `sg-complete` → `sg-new`로 이어진다.

```text
수동 진입                      자동 체인 (sg-execute부터 sg-next가 주도)
sg-start / sg-new                          ┌── [sg-tdd] ─ [sg-review] ─ [sg-learn] ──┐
   → sg-explore → sg-plan ── sg-execute ───┤   (선택 — sg-toggle-* 로 생략 가능)      ├── sg-ship
                      ▲                     └─────────────────────────────────────────┘     │
                      └── 다음 phase / 교훈 주입 ─────────────────────────────────────┘     │
                                                              마일스톤 완료 ── sg-complete ─→ sg-new
[ … ] = 선택 단계 — sg-toggle-tdd / sg-toggle-review / sg-toggle-learn 로 생략
```
...
flowchart LR
    ...
    EXE -.->|"tdd_mode off (기본)"| REV
    EXE -.->|"skip_review"| LRN
    REV -.->|"skip_learn"| SHIP

    SHIP -->|"다음 phase"| PL
    SHIP -->|"마일스톤 완료"| CMP["sg-complete"]
    CMP --> NEW["sg-new"]
    NEW -.->|"다음 마일스톤"| PL
    LRN -.->|"교훈 → 다음 plan"| PL
...
> 실선 화살표는 기본 경로, 점선 화살표는 단계를 끄면 활성화되는 생략 경로다(`tdd_mode`는 기본 off라 `sg-tdd`는 켜야 실행된다). 노란 점선 노드가 토글 가능한 선택 단계다.
...
| `/super-gsd:sg-toggle-tdd` | `sg-tdd` 단계 ON/OFF 토글 (`super_gsd.tdd_mode`) — `on`/`off` 또는 인자 없이 플립 | TDD 검증 게이트를 켜거나 끌 때 |
| `/super-gsd:sg-toggle-review` | `sg-review` 단계 ON/OFF 토글 (`super_gsd.skip_review`) — `on`/`off` 또는 인자 없이 플립 | 코드 리뷰를 워크플로우에 포함/생략할 때 |
| `/super-gsd:sg-toggle-learn` | `sg-learn` 단계 ON/OFF 토글 (`super_gsd.skip_learn`) — `on`/`off` 또는 인자 없이 플립 | 회고를 워크플로우에 포함/생략할 때 |
...
| 스킬 커버리지 | ✅ `skills/`에 25/25 | ⚠️ `.agents/skills/`에 15/25 | ⚠️ `.agents/skills/`에 15/25 |
...
`.codex/hooks.json` (Stop 및 PreToolUse 훅), `hooks/` (Node.js .cjs 스크립트), `.agents/skills/` (`$sg-*` 스킬)을 설치한다. 사용 가능한 스킬: `$sg-start`, `$sg-plan`, `$sg-execute`, `$sg-parallel-execute`, `$sg-review`, `$sg-learn`, `$sg-retro`, `$sg-ship`, `$sg-status`, `$sg-next`, `$sg-tdd`, `$sg-toggle-tdd`, `$sg-toggle-review`, `$sg-toggle-learn`, `$sg-setup`. 전체 워크플로우는 `AGENTS.md`를 참조한다.
...
- **Phase 3 — sg- 명령 세트 및 README (완료):** 전체 GSD → Superpowers → sg-retro 사이클을 커버하는 14개 명령 인터페이스와 업데이트된 문서 제공. *(이후 phase에서 25개 명령으로 확장 — 현재 목록은 Commands 테이블 참조.)*
