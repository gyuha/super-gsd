...
## Current State (v2.4 shipped 2026-05-26)

- **Skills**: `skills/sg-*/SKILL.md` 18개 — 15개 워크플로우 명령(sg-next 포함) + sg-retro + sg-ui-plan + sg-parallel-execute
...
- **Python 의존성 제거 완료**: `hooks/*.py` 4개 삭제, 모든 SKILL.md `python3` 호출 → `node -e`/`node hooks/*.cjs` 교체
- **Rule runner**: `hooks/rule_runner.cjs` — `.claude/sg-rule.*.local.md` 규칙 평가 (PreToolUse)
- **Lessons ranking**: `hooks/lessons_ranker.cjs` — weighted top-N (0.4×freq + 0.4×recency + 0.2×severity), sg-plan 진입 시 자동 노출
- **Dependencies**: GSD + Superpowers 필수. Node.js 18+ 필수 (Claude Code 자체가 요구하므로 사실상 보장). Hookify Optional.
- **Version**: 0.0.38

## Completed Milestone: v2.4 Hooks Node Migration (2026-05-26)

**Delivered:** Python 의존성 완전 제거. Node.js 18+ 단독 실행. `hooks/*.py` 4개 삭제, `hooks/*.cjs` 4개 신규 작성, 3개 hook config 교체, 8개 SKILL.md python3 → node 교체, 문서 전면 갱신. Plugin version 0.0.38.

**Next milestone candidates:** v2.5 documentation cleanup (README.ko.md 전체 동기화), quick tasks 정리, Multi-Platform enhancements
...
---
*Last updated: 2026-05-26 after v2.4 milestone — Python 의존성 완전 제거, Node.js 18+ 단독 실행 달성. Version 0.0.38.*
