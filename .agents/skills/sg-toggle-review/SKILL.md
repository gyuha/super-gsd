...
description: Toggle the sg-review stage on or off in the super-gsd workflow by flipping super_gsd.skip_review in .planning/config.json. Accepts on|off, or no argument to flip.
...
2. **Read-modify-write config.json (macOS compatible — node only, no jq).**
   The underlying flag is `skip_review` (inverted: review ON ⇒ skip_review=false).
   ```bash
...
3. **Report the new state in the user's language.**
   Surface the result as prose in the user's input language. Keep the command token `$sg-review`, the flag name `skip_review`, and the stage sequence tokens (`execute → … → ship`) verbatim.
